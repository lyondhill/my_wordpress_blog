<?php // Do not delete these lines
	if ('comments.php' == basename($_SERVER['SCRIPT_FILENAME']))
		...
		/* This variable is for alternating comment background which is mandatory*/
		$oddcomment = 'alt';
?>
...
<?php if ($comments) : ?>
	...
	<?php foreach ($comments as $comment) : ?>
		...
		<li id="comment-<?php comment_ID() ?>" class="<?php echo $oddcomment; /* Style differently if comment author is blog author */ if ($comment->comment_author_email == get_the_author_email()) { echo ' authorcomment'; } ?>">
			<div class="cmtinfo"><small class="commentmetadata">
			<?php echo get_avatar( $comment, 32 ); ?>
			<a href="#comment-<?php comment_ID() ?>" title="">#</a> </small><cite><?php comment_author_link() ?></cite><em>on <?php comment_date('d M Y') ?> at <?php comment_time() ?> <?php edit_comment_link('edit this','',''); ?></em></div>
